...
class RepositoryContractTests(unittest.TestCase):
    ...
    def _assert_release_workflow_contract(self, ci: str, release: str) -> None:
        ...
        for workflow_name, workflow_steps in [("CI", ci_steps), ("release", verify_steps)]:
            ...
            fixture_body = named_step(workflow_steps, "Validate v3.1 fixtures against schema")
            ...
            self.assertLess(
                install_position,
                headers.index("name: Validate v3.1 fixtures against schema"),
                workflow_name,
            )
            build_positions = [
                index
    ...
    def test_version_is_consistent(self) -> None:
        self.assertEqual(self.read("VERSION"), "3.1.0\n")
    ...
    def test_install_docs_name_exact_assets_platforms_and_layout(self) -> None:
        ...
        install = self.read("docs/INSTALL.md")
        for phrase in [
            "image-intake-router-3.1.0.tgz",
            "image-intake-router-3.1.0.tgz.sha256",
        ]:
        ...
        for phrase in [
            ...
            "Get-FileHash",
            "sha256sum -c",
            "image-intake-router-3.1.0/image-intake-router/",
            "<OPENCLAW_SKILLS_DIR>",
        ]:
    ...
    def test_docs_cover_required_operations(self) -> None:
        expected = {
            ...
            "docs/UPGRADING.md": ["更新", "3.1.0", "回滚", "数据库"],
    ...
    def test_release_contract_rejects_fixture_gate_after_archive_build(self) -> None:
        ...
        release = self.read(".github/workflows/release.yml")
        fixture_step = (
            "      - name: Validate v3.1 fixtures against schema\n"
